...
${DIGCOLPROC_HOME}util/ftp.sh "$ftp_script" "synchronize remote -mirror --delete -criteria=size $fileSet_windows\.$targetLevel $archiveID/.$targetLevel" "$flow_ftp_connection" "$log"
rc=$?
if [[ $rc != 0 ]] ; then
    exit -1
fi
